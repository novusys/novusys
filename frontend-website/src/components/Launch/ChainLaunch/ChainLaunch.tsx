...
const ChainLaunch: React.FC<ChainLaunchProps> = ({ cid, custodial, recoverySigners, securityFeatures }) => {
  ...
  const op = () =>
    getOp(
      user?.sub,
      address != undefined ? address : "0x6d06Eb861809551087F5b37272f36ceF459C5338",
  ...
  const savingsSetup = () => {
    if (securityFeatures["savings"]["enabled"]) {
      ...
      approveSavings(
        ...
        new ethers.providers.JsonRpcProvider(chains[cid].bundler),
        securityFeatures["savings"]["address"],
        securityFeatures["savings"]["savings_percent"],
        true,
        chains[cid].paymasterAddress
      );
    }
    ...
    <>
      <div className={styles["out__container"]}>
        ...
        <div
          ...
          onClick={() => {
            window.open(chains[cid].explorer + "address/" + usrAddress, "_blank");
          }}
        >
          {
        ...
        {alreadyDeployed == "0x" ? (
          <div className={styles["right__container"]}>
            <BlurPaper>
              {status == "launch contract" ? (
                <div className={styles["status__blurb"]} onClick={() => txn()}>
                  Launch Contract
                </div>
              ) : (
                <></>
              ...
              {status == "processing" ? <div className={styles["status__blurb__loading"]}>Processing Txn....</div> : <></>}
              {status == "fail" ? (
                <div
                  className={styles["status__blurb"]}
                  onClick={() => window.open(chains[cid].explorer + "tx/" + transaction, "_blank")}
                >
                ...
                isConnected ? (
                  <div className={styles["status__blurb"]} onClick={() => {}}>
                    {usrAddress == "" ? (
                      ...
                      <FundAddress
                        explorer={chains[cid].explorer}
                        cid={cid}
                        value={chains[cid].launchPrice}
                        address={usrAddress}
                        setConfirm={() => {
                          setStatus("launch contract");
                        }}
          ...
          <div className={styles["right__container"]}>
            <BlurPaper>
              <div className={styles["status__blurb"]}>Already Deployed!</div>
            </BlurPaper>
          </div>
        )}
      </div>
      {initTxn != "" ? (
        <div className={styles["signer__init"]}>
          {" "}
          Signer Initalization Transaction:{" "}
          <a
            onClick={() => {
              window.open(chains[cid].explorer + "tx/" + initTxn), "_blank";
            }}
          >
            {initTxn}
          </a>
      ...
      {initSavings != "" ? (
        <div className={styles["signer__init"]}>
          {" "}
          Savings Initalization Transaction:{" "}
          <a
            onClick={() => {
              window.open(chains[cid].explorer + "tx/" + initTxn), "_blank";
            }}
      ...
      {initPauser != "" ? (
        <div className={styles["signer__init"]}>
          {" "}
          Pauser Initalization Transaction:{" "}
          <a
            onClick={() => {
              window.open(chains[cid].explorer + "tx/" + initTxn), "_blank";
            }}
          ...
          With novusys admin address: 0x3C445B5174EED7a7f267cd37b87Fec13c59f4128
        </div>
      ) : (
        <></>
      )}
